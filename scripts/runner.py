...
LOG_DIR = "../logs"
LOG_PATH = os.path.join(LOG_DIR, "english_latency_raw.csv")
PROGRESS_FILE = "../logs/english_progress.json"
...
def run_benchmark():
    ...
    for LANGUAGE in LANGUAGES:
        ...
        end_index = start_index + MAX_SENTENCES

        batch_lines = lines[start_index:end_index]

        print(f"Processing from {start_index} to {end_index}")
        ...
        with open(LOG_PATH, "a", newline="", encoding="utf-8") as log_file:
            ...
            for i, text in enumerate(batch_lines, start=start_index):
                ...
                ttfa_ms = round(latency * 1000) if latency is not None else ""
                writer.writerow([
                    timestamp,
                    LANGUAGE,
                    text,
