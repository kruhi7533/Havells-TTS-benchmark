...
DATASET_PATH = "../dataset/english.txt"
    ...
    for i in range(0, len(dataset), concurrency_level):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency_level) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    success, latency = future.result(timeout=30)
...
def eleven_wrapper(text):
    success, latency, _ = generate_elevenlabs_audio(text, VOICE_ID,None)
    return success, latency

def sarvam_wrapper(text):
    success, latency, _ = generate_sarvam_audio(text,None)
    return success, latency
...
def run_concurrency_tests():
    ...
    for level in CONCURRENCY_LEVELS:
        ...
        for model_name, func in [("ElevenLabs", eleven_wrapper), ("Sarvam", sarvam_wrapper)]:
            latencies, failures = run_test_for_model(model_name, func, dataset, level)
